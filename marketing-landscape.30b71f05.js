...
})({"fEiJR":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "55318b9730b71f05";
"use strict";
...
},{}],"l1bH3":[function(require,module,exports) {
...
document.addEventListener("DOMContentLoaded", async ()=>{
    try {
        ...
        if (response.user.accountNiche != "" && response.user.accountNicheStatus != "Active") document.querySelectorAll(".niche-icon").forEach((icon)=>{
            if (icon.getAttribute("data-plan-type") == response.user.accountNiche) {
                // icon.style.display = "none";
                icon.classList.add("hidden");
                const siblingicon = icon.nextElementSibling;
                if (siblingicon && siblingicon.getAttribute("data-plan-type").includes(`${response.user.accountNiche}-cancel`)) {
                    ...
                    siblingicon.addEventListener("click", async (e)=>{
                        ...
                        const response = await fetchDataFromRoute("users", "patch", {
                            accountNiche: pTYPE.split("-").includes("cancel") ? "cancel" : null
                        }, null);
                        console.log(response);
        ...
        if (response.user.accountNicheStatus == "Active") document.querySelectorAll(".niche-icon").forEach((icon)=>{
            if (icon.getAttribute("data-plan-type") == response.user.accountNiche) {
                icon.classList.add("hidden");
        ...
        if (response.user.accountSmartLink == "Pending" && response.user.accountSmartLinkAmount <= 0) {
            const classADV = document.querySelector(".smartlink-fee-req").className;
            const parEL = document.querySelector(".smartlink-fee-req").parentElement;
            document.querySelector(".smartlink-fee-req").remove();
            const slaBTN = document.createElement("button");
            slaBTN.className = classADV.split(" ").filter((e)=>e != "smartlink-fee-req").join(" ");
        ...
        if (response.user.accountAdvert == "Pending" && response.user.accountAdvertAmount <= 0) {
            const classADV = document.querySelector(".advert-fee-req").className;
            const parEL = document.querySelector(".advert-fee-req").parentElement;
            document.querySelector(".advert-fee-req").remove();
            const slaBTN = document.createElement("button");
            slaBTN.className = classADV.split(" ").filter((e)=>e != "advert-fee-req").join(" ");
        ...
        if (response.user.accountSmartLinkAmount >= 0 && response.user.accountSmartLink == "Pending") {
            const classADV = document.querySelector(".smartlink-fee-req").className;
            const parEL = document.querySelector(".smartlink-fee-req").parentElement;
            document.querySelector(".smartlink-fee-req").remove();
        ...
        if (response.user.accountAdvertAmount >= 0 && response.user.accountAdvert == "Pending") {
            const classADV = document.querySelector(".advert-fee-req").className;
            const parEL = document.querySelector(".advert-fee-req").parentElement;
            document.querySelector(".advert-fee-req").remove();
            const slaBTN = document.createElement("a");
...
document.querySelector(".advert-fee-req").addEventListener("click", async (e)=>{
    e.preventDefault();
    const response = await fetchDataFromRoute("users", "patch", {
...
document.querySelector(".smartlink-fee-req").addEventListener("click", async (e)=>{
    e.preventDefault();
    const response = await fetchDataFromRoute("users", "patch", {
...
},{"../index":"8lqZg"}]},["fEiJR","l1bH3"], "l1bH3", "parcelRequire096c")

//# sourceMappingURL=marketing-landscape.30b71f05.js.map
