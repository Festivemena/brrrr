...
})({"iuWiF":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "75bf426dc0a47f44";
"use strict";
...
},{}],"d1x64":[function(require,module,exports) {
var _index = require("../index");
const { fetchDataFromRoute } = _index;
document.addEventListener("DOMContentLoaded", async (e)=>{
    const loaderContainer1 = document.getElementById("loaderContainer");
    const plansContainer = document.querySelector("#plans-container");
    const plansContainerMobile = document.querySelector("#plans-container-mobile");
    e.preventDefault();
    try {
        const { plans } = await fetchDataFromRoute("plans", "get");
        console.log(plans);
        if (plans.length < 1) {
            loaderContainer1.classList.add("hidden");
            return;
        }
        loaderContainer1.classList.add("hidden");
        for(const planName in plans)if (plans.hasOwnProperty(planName)) {
            ...
            if (Array.isArray(planData) && planData.length > 0) {
                ...
                planData.forEach(async (item, index)=>{
                    console.log(`  Fetching data ${index + 1} for ${planName}: ${item}`);
...
async function loadDOMItems(container, data) {
    ...
    div1.classList.add("mt-2", "w-full", "bg-darkbg1", "flex", "flex-col", "justify-center", "items-start", "px-2", "mb-1");
    function editName(planName) {
        const modifiedName = planName.replace("Plan", "").trim();
        const isolatedWord = modifiedName.split(/(?=[A-Z])/).join(" - ").trim();
    ...
    const pE = `
      <p class="flex flex-row justify-between space-x-1 w-full items-center">
         ...
         <span class="font-medium text-gray-100"
            >${("0" + new Date(data.createdAt).getDate()).slice(-2)}/${("0" + (new Date(data.createdAt).getMonth() + 1)).slice(-2)}/${new Date(data.createdAt).getFullYear().toString().slice(-2)}</span
         >
      </p>
   `;
    ...
    div3.classList.add("mt-4", "flex", "flex-row", "mx-auto", "space-x-6", "justify-between", "items-center", "w-full", "px-4", "py-2");
    const span1 = document.createElement("span");
    span1.textContent = `${accountCurrency} ${data.investAmount}`;
    span1.classList.add("text-lg", "font-medium", "text-gray-100");
    const span3 = document.createElement("span");
    span3.textContent = `${accountCurrency} ${Number(data.currentAmount).toFixed(2)}`;
...
async function loadDOMItemsMobile(container, data) {
    ...
    const { accountCurrency } = user;
    const div1 = document.createElement("div");
    div1.className = "mt-2 w-full bg-darkbg1 flex flex-col justify-center items-start px-2";
    function editName(planName) {
        const modifiedName = planName.replace("Plan", "").trim();
        const isolatedWord = modifiedName.split(/(?=[A-Z])/).join(" - ").trim();
        return isolatedWord; // Output: Starter - Growth
   ...
   `;
    ...
    h3.textContent = `${editName(data.planName)}`;
    const div3 = document.createElement("div");
    div3.className = "mt-4 flex flex-row mx-auto space-x-6 justify-between items-center w-full px-4 py-2";
    const span1 = document.createElement("span");
    span1.textContent = `${accountCurrency} ${data.investAmount}`;
    span1.classList.add("text-lg", "font-medium", "text-gray-100");
    const span3 = document.createElement("span");
...
},{"../index":"8lqZg"}]},["iuWiF","d1x64"], "d1x64", "parcelRequire096c")

//# sourceMappingURL=account-status.c0a47f44.js.map
